...
// 存储viewpoint的各种信息，包括位置、朝向、可见的frontier数量等
struct Viewpoint {
...
public:
  ...
  void searchFrontiers(Eigen::Vector3d cur_pos); // 搜索frontier
  void computeFrontiersToVisit(Eigen::Vector3d cur_pos); // 计算哪些frontier需要访问
  void computeNormal(const vector<Vector3d> &point_cloud,
                     vector<Vector3d> &normals, Vector3d center); //算范数？
  void getFrontiers(vector<vector<Vector3d>> &clusters); // 获取frontier
  void getFrontierDivision(vector<vector<Eigen::Vector3d>> &division); // 获取frontier划分
  void getDormantFrontiers(vector<vector<Vector3d>> &clusters); // 获取休眠的frontier
  void getFrontierBoxes(vector<pair<Vector3d, Vector3d>> &boxes); // 获取frontier的边界框
  ...
  void getTopViewpointsInfo(const Vector3d &cur_pos, vector<Vector3d> &points,
                            vector<double> &yaws, vector<Vector3d> &averages); // 获取frontier的视点
  void getTop5ViewPoints(vector<vector<Eigen::Vector3d>> &clusters); // 获取frontier的前5好视点
  ...
  void getViewpointsInfo(const Vector3d &cur_pos, const vector<int> &ids,
                         ...
                         vector<vector<double>> &yaws); // 获取frontier最佳视点信息
  void updateFrontierCostMatrix();
  void clusterFrontiers(const Eigen::Vector3d &cur_pos, bool &neighbor); //前沿点聚类
  ...
  void getCheckTourCostMatrix(const Vector3d &cur_pos, const Vector3d &cur_vel,
                              ...
                              Eigen::MatrixXd &mat);// 获取frontier的check tour
  void getSingleCellCostMatrix(const Vector3d &cur_pos, const Vector3d &cur_vel,
                               ...
                               Eigen::MatrixXd &mat); // 获取单个frontier的cost matrix
